...
class StockPicking(models.Model):
    ...
    is_field_invisible = fields.Boolean(compute='_compute_is_field_visible', string="Is Field Visible", store=True)

    @api.depends('transfer_count', 'is_transfer')
    def _compute_is_field_visible(self):
        for record in self:
            record.is_field_invisible =  (record.transfer_count == 0 or not record.is_transfer)
